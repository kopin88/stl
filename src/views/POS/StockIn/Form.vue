...
<v-ons-page>
    ...
    <v-ons-card>
        <v-ons-list>
          ...
          <v-ons-list-item>
            <div class="left">
              <v-ons-icon icon="md-check" class="list-item__icon text__color" v-if="newItem.name != null"></v-ons-icon>
              <v-ons-icon icon="md-minus-circle-outline" style="color: #FB8C00;" class="list-item__icon" v-else></v-ons-icon>
            </div>
            <div class="center">
                <multiselect v-model="newItem" style="width:100%" placeholder="Select Item" label="name" track-by="id" :options="option.items" :option-height="104" :custom-label="ItemSelect" :show-labels="true">
